...
namespace NotificationPortal.Data.Core
{
    public static class FirebaseMessagingService
    {
        public static async Task<ChallengeNotification> SendMessage(
            ChallengeEntry challenge, string firebaseConfigurationJson)
        {
            ...
            if (string.IsNullOrEmpty(firebaseConfigurationJson))
            {
                return challengeNotification with { FirebaseResponse =
                    "Mock response - Firebase configuration missing" };
            }
            ...
            if (FirebaseApp.DefaultInstance == null)
            {
                ...
                {
                    Credential = GoogleCredential.FromJson(firebaseConfigurationJson)
                });
            }
        ...
        public static async Task<ChallengeNotification> SendMessageResponseToChallenge(
            ChallengeEntry challenge, NotificationType response, string firebaseConfigurationJson)
        {
            ...
            if (string.IsNullOrEmpty(firebaseConfigurationJson))
            {
                return challengeNotificaton with { FirebaseResponse =
                    "Mock response - Firebase configuration missing" };
            }
