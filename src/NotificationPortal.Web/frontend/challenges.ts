...
function findStatusCell(notificationId) {
  ...
  for (const statusCell of Array.from(allStatusCells)) {
    if (
      statusCell instanceof HTMLElement &&
      Number(statusCell.dataset.id) === notificationId
    )
      return statusCell;
  }
...
connection.on(
  "NewChallengeIssued",
  function (challengeId, communityName, fromPlayer, toPlayer, status, date) {
    console.log(
      "Challenge received: ",
      challengeId,
      communityName,
...
connection.on("ChallengeStatusChanged", function (challengeId, newStatus) {
  ...
  for (const acceptButton of Array.from(acceptButtons)) {
    if (
      ...
      Number(acceptButton.dataset.id) === challengeId
    ) {
  ...
  for (const declineButton of Array.from(declineButtons)) {
    if (
      ...
      Number(declineButton.dataset.id) === challengeId
    ) {
      if (!buttonsShouldBeVisible) {
